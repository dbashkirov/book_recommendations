...
def message_handle(message):
    global book_id
    if (
        message.text[:7] == "/choose"
        and len(message.text) > 7
        and message.text[7:].isdigit()
    ):
        ...
        message.text[:9] == "/readlist"
        and len(message.text) > 9
        and message.text[9:].isdigit()
    ):
        ...
        if int(message.text[7:]) > 10000 or int(message.text[7:]) < 1:
            ...
            bot.send_message(
                message.chat.id, "Книга была удалена из Вашего списка читаемого."
            )
            return
    elif message.text == "О нас":
        bot.send_message(
            message.from_user.id,
            "Башкиров Данил - разработчик системы рекомендаций\nПозняк Даниил - разработчик бота\nВальба "
        ...
        bot.send_message(
            message.from_user.id, "Здесь будет краткая информация о возможностях бота."
        )
...
def get_sex(message):
    ...
    if (
        sex[message.from_user.id] != "Мужской"
        and sex[message.from_user.id] != "Женский"
    ):
        msg = bot.send_message(message.from_user.id, "Пожалуйста, выберите пол.")
        bot.register_next_step_handler(msg, get_sex)
        return
    bot.send_message(
        message.from_user.id, "Ваш возраст:", reply_markup=types.ReplyKeyboardRemove()
    )
...
def find_book(message):
    ...
    with open(name, "r", encoding="utf8") as file:
        ...
        if num_of_books > 15:
            bot.send_message(
                message.chat.id,
                "Получено слишком много результатов поиска (>15). \n"
    ...
    if not found_flag and num_of_books == 0:
        msg = bot.send_message(
            message.from_user.id,
            "К сожалению, не удалось найти книгу. Попробуйте еще раз.",
        )
        bot.register_next_step_handler(msg, find_book)
...
def get_score(message):
    ...
    if score == "В список читаемого":
        ...
        with open(name, "r", encoding="windows-1251") as file:
            ...
            for j, row in enumerate(reader):
                if row[0] == str(book_id[message.from_user.id]):
                    msg = bot.send_message(
                        message.from_user.id,
                        "Книга уже находится в Вашем списке читаемого.",
                    )
        ...
        with open(name, "a", newline="") as csv_file:
            ...
            for line in written:
                writer.writerow(line)
        bot.send_message(
            message.from_user.id, "Книга добавлена в Ваш список читаемого."
        )
        time.sleep(0.3)
        make_keyboard(message)
        return
    ...
    if score > 5 or score < 1:
        msg = bot.send_message(
            message.from_user.id, "Оценка должна быть в диапазоне от 1 до 5."
        )
        bot.register_next_step_handler(msg, get_score)
        return
    score_flag = False
...
def next_reg(message):
    ...
    if reg_iter[message.from_user.id] == 10:
        ...
        bot.send_message(
            ...
            "чтобы рекомендации были точнее.",
...
def make_keyboard(message):
    ...
    keyboard_yes.add(key_help, key_about)
    bot.send_message(
        message.from_user.id, "Выберите желаемое действие.", reply_markup=keyboard_yes
    )
        ...
        with open("../data/raw/ratings.csv", "r", encoding="windows-1251") as file:
            ...
            for j, row in enumerate(reader):
                if row[0] == str(user_id) and row[1] == str(needed_id):
                    rated_flag = True
        with open(
            "users/" + str(message.from_user.id) + ".csv", "r", encoding="windows-1251"
        ) as file:
            a = file.read()
